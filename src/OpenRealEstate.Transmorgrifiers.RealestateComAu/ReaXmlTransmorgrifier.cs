...
namespace OpenRealEstate.Transmorgrifiers.RealEstateComAu
{
    public class ReaXmlTransmorgrifier : ITransmorgrifier
    {
        ...
        {
            get => _defaultCultureInfo ?? new CultureInfo("en-au");
        ...
        private static string RemoveByteOrderMark(string text)
        {
            ...
            for (var i = byteOrderMark.Length - 1; i >= 0; i--)
            {
        ...
        /// <summary>
        ///   The display address is the nicely formatted address which we can display to a person.
        ///   Sometimes, the agent might not want to display anything secret to people, like
        ///   street number or name, etc. So this is autogenerated for REA Xml data.
        /// </summary>
        private static void CalculateDisplayAddress(XElement addressElement,
                                                    Address address)
        {
            if (addressElement == null)
            {
                throw new ArgumentNullException(nameof(addressElement));
            }
            ...
            /* If we are not suppose to show the street, then we just show the 
               ...
               NOTE: Display (attribute) has the values of:
                     - display="true" : show address
                     - display="false" : hide the address
                     - display attribute is missing: _show_ the address (if you don't give us an attribute, 
                       we're assuming you don't care, which is to show the property.
                     - display="not true or false" : bad value, so false.
            */
            var isStreetDisplayedText = addressElement.AttributeValueOrDefault("display");
            var isStreetDisplayed = string.IsNullOrWhiteSpace(isStreetDisplayedText) || // No value, then we assume TRUE!
        ...
        private static void ExtractLatitudeLongitudes(XContainer document,
                                                      Address address)
        {
            Guard.AgainstNull(document);
        ...
        private static void ExtractAgents(XContainer document,
                                          Listing listing)
        {
            Guard.AgainstNull(document);
            Guard.AgainstNull(listing);
            ...
            foreach (var agentElement in agentElements)
            {
                ...
                if (!agents.Any(x => x.Name.Equals(agent.Name, StringComparison.OrdinalIgnoreCase)))
                {
        ...
        private static void ExtractFeatures(XContainer document,
                                            Listing listing)
        {
            Guard.AgainstNull(document);
            Guard.AgainstNull(listing);
            ...
            foreach (var feature in new[]
                                    ...
                                    .Select(ExtractBooleanFeatures)
                                    .Where(features => features.Any())
                                                         ...
                                                         string delimeter = "-")
        {
            ...
            if (validValues.Contains(type, StringComparer.OrdinalIgnoreCase))
            {
        ...
        private static void ExtractOtherFeatures(XElement document,
                                                 ISet<string> tags)
        {
            Guard.AgainstNull(document);
            Guard.AgainstNull(tags);
        ...
        private static ISet<string> ExtractBooleanFeatures(XElement document)
        {
            ...
            Parallel.ForEach(XmlFeatureHelpers.OtherFeatureNames,
                             possibleFeature =>
                             {
        ...
        // NOTE: This is a extracting a collection. As such, we hard-copy the resultant collection
        //       over to the listing. No guessing or matching.
        private static void ExtractInspectionTimes(XContainer document,
                                                   Listing listing)
        {
            Guard.AgainstNull(document);
            Guard.AgainstNull(listing);
            ...
            foreach (var inspectionElement in inspectionElements)
            {
                ...
                var data = inspectionElement.Value.Split(new[]
                                                         {
                                                             ' '
                                                         },
                                                         StringSplitOptions.RemoveEmptyEntries);
                ...
                DateTime.TryParse($"{data[0]} {data[1]}", out var inspectionStartsOn);
                DateTime.TryParse($"{data[0]} {data[3]}", out var inspectionEndsOn);
        ...
        // NOTE: This is a extracting a collection. As such, we hard-copy the resultant collection
        //       over to the listing. No guessing or matching.
        private static void ExtractImages(XContainer document,
                                          Listing listing)
        {
            Guard.AgainstNull(document);
            Guard.AgainstNull(listing);
        ...
        // NOTE: This is a extracting a collection. As such, we hard-copy the resultant collection
        //       over to the listing. No guessing or matching.
        private static void ExtractFloorPlans(XContainer document,
                                              Listing listing)
        {
            Guard.AgainstNull(document);
            Guard.AgainstNull(listing);
        ...
        // NOTE: This is a extracting a collection. As such, we hard-copy the resultant collection
        //       over to the listing. No guessing or matching.
        private static void ExtractVideos(XElement document,
                                          Listing listing)
        {
            Guard.AgainstNull(document);
            Guard.AgainstNull(listing);
        ...
        */
        ...
        private static void ExtractSoldOn(XElement document,
                                          SalePricing salePricing)
        {
            Guard.AgainstNull(document);
        ...
        private static void ExtractLandDetails(XContainer document,
                                               Listing listing)
        {
            Guard.AgainstNull(document);
            Guard.AgainstNull(listing);
        ...
        private static void ExtractExternalLinks(XContainer document,
                                                 Listing listing)
        {
            Guard.AgainstNull(document);
            Guard.AgainstNull(listing);
        ...
        private static void ExtractResidentialNewConstruction(XElement document,
                                                              ResidentialListing listing)
        {
            Guard.AgainstNull(document);
            Guard.AgainstNull(listing);
        ...
        private static void ExtractRentalData(RentalListing rentalListing,
                                              XElement document,
                                              CultureInfo cultureInfo)
        {
                                                 ...
                                                 CultureInfo cultureInfo)
        {
            ...
            foreach (var rentElement in rentElements)
            {
                ...
                if (frequency.Equals("week", StringComparison.OrdinalIgnoreCase) ||
                    frequency.Equals("weekly", StringComparison.OrdinalIgnoreCase))
                {
                    rentalPricing.PaymentFrequencyType = PaymentFrequencyType.Weekly;
                }
                else if (frequency.Equals("month", StringComparison.OrdinalIgnoreCase) ||
        ...
        private static OpenRealEstate.Core.Land.CategoryType ExtractLandCategoryType(XElement xElement)
        {
            ...
            return string.IsNullOrWhiteSpace(category)
                       ? OpenRealEstate.Core.Land.CategoryType.Unknown
        ...
        private static OpenRealEstate.Core.Rural.CategoryType ExtractRuralCategoryType(XContainer document)
        {
            ...
            {
                return OpenRealEstate.Core.Rural.CategoryType.Unknown;
            }

            var categoryValue = categoryElement.AttributeValueOrDefault("name");
            return string.IsNullOrWhiteSpace(categoryValue)
                       ? OpenRealEstate.Core.Rural.CategoryType.Unknown
                       : OpenRealEstate.Core.Rural.CategoryTypeHelpers.ToCategoryType(categoryValue);
        }
        ...
        private static void ExtractRuralNewConstruction(XElement document,
                                                        Listing listing)
        {
            Guard.AgainstNull(document);
            Guard.AgainstNull(listing);
