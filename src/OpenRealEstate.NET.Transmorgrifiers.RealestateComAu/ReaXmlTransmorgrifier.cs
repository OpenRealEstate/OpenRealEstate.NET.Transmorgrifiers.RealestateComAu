...
using OpenRealEstate.NET.Transmorgrifiers.RealestateComAu.Extensions;
using CategoryTypeHelpers = OpenRealEstate.NET.Core.Land.CategoryTypeHelpers;
...
namespace OpenRealEstate.NET.Transmorgrifiers.RealestateComAu
{
    public class ReaXmlTransmorgrifier : ITransmorgrifier
    {
                                                     ...
                                                     Address address)
        {
            ...
            address.DisplayAddress = address.ToString(isStreetDisplayed, false, false);
        }
        ...
        */
        ...
        private static NET.Core.Land.CategoryType ExtractLandCategoryType(XElement xElement)
        {
            var category = xElement.ValueOrDefault("landCategory", "name");
            return string.IsNullOrWhiteSpace(category)
                       ? NET.Core.Land.CategoryType.Unknown
        ...
        private static NET.Core.Rural.CategoryType ExtractRuralCategoryType(XContainer document)
        {
            Guard.AgainstNull(document);
            ...
            {
                return NET.Core.Rural.CategoryType.Unknown;
            }

            var categoryValue = categoryElement.AttributeValueOrDefault("name");
            return string.IsNullOrWhiteSpace(categoryValue)
                       ? NET.Core.Rural.CategoryType.Unknown
                       : NET.Core.Rural.CategoryTypeHelpers.ToCategoryType(categoryValue);
        }
